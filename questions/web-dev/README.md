
# 🌐 Web Development

[![Last Updated](https://img.shields.io/badge/Updated-March%202026-green.svg)](#)


---

## 📂 Topics

| # | Topic | Questions | Link |
|---|-------|-----------|------|
| 1 | HTML & CSS | 0 | [Go →](./html-css.md) |
| 2 | JavaScript (DOM, Events) | 0 | [Go →](./javascript-dom.md) |
| 3 | React.js | 0 | [Go →](./reactjs.md) |
| 4 | Node.js | 0 | [Go →](./nodejs.md) |
| 5 | Express.js | 0 | [Go →](./expressjs.md) |
| 6 | REST APIs | 0 | [Go →](./rest-apis.md) |
| 7 | GraphQL | 0 | [Go →](./graphql.md) |
| 8 | Next.js | 0 | [Go →](./nextjs.md) |
| 9 | Angular (Scenario-Based, 5-9 YOE) | 10 | [Go →](./02-angular-scenario-based.md) |
| 10 | Vue.js | 0 | [Go →](./vuejs.md) |
| 11 | Performance & Optimization | 0 | [Go →](./performance.md) |
| 12 | Browser Internals | 0 | [Go →](./browser-internals.md) |

---

[← Back to Home](../../README.md)
]]>
